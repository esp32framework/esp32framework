...
impl<'a> DS3231<'a> {
    ...
    fn parse_component(&self, read_value: u8, component: DateTimeComponent) -> u8 {
        match component {
            ...
            DateTimeComponent::Hour => {
                self.bcd_to_decimal(read_value & self.mode.get_read_bitmask())
            }
    ...
    pub fn set_alarm_1(&mut self, alarm_rate: Alarm1Rate) -> Result<(), I2CError> {
        match alarm_rate {
            Alarm1Rate::EverySecond => {
                self._set_alarm_1(ALARM_MSB_ON, ALARM_MSB_ON, ALARM_MSB_ON, ALARM_MSB_ON)
            }
            Alarm1Rate::Seconds(secs) => {
                self._set_alarm_1(ALARM_MSB_ON, ALARM_MSB_ON, ALARM_MSB_ON, secs)
            }
            Alarm1Rate::MinutesAndSeconds(mins, secs) => {
                self._set_alarm_1(ALARM_MSB_ON, ALARM_MSB_ON, mins, secs)
            }
            Alarm1Rate::HoursMinutesAndSeconds(hr, mins, secs) => {
                self._set_alarm_1(ALARM_MSB_ON, hr, mins, secs)
            }
            Alarm1Rate::DateHoursMinutesAndSeconds(date, hr, mins, secs) => {
                self._set_alarm_1(date & 0xBF, hr, mins, secs)
            }
            Alarm1Rate::DayHoursMinutesAndSeconds(day, hr, mins, secs) => {
                self._set_alarm_1(day | 0x40, hr, mins, secs)
            }
        }
    }
    ...
    pub fn set_alarm_2(&mut self, alarm_rate: Alarm2Rate) -> Result<(), I2CError> {
        match alarm_rate {
            ...
            Alarm2Rate::DateHoursAndMinutes(date, hr, mins) => {
                self._set_alarm_2(date & 0xBF, hr, mins)
            }
            Alarm2Rate::DayHoursAndMinutes(day, hr, mins) => {
                self._set_alarm_2(day | 0x40, hr, mins)
            }
        }
    }
...
impl DateTimeComponent {
    ...
    pub fn is_between_boundaries(&self, val: u8, hour_mode: HourMode) -> bool {
        match self {
            ...
            DateTimeComponent::Hour => match hour_mode {
