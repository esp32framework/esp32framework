...
impl<'a> _TimerDriver<'a> {
    ...
    fn micro_to_counter(&self, micro_seconds: u64) -> u64 {
        (micro_seconds as u128* self.driver.tick_hz() as u128).min(u64::MAX as u128) as u64 / MICRO_IN_SEC
    }
...
#[cfg(test)]
mod test{
    use esp_idf_svc::{hal::{delay::FreeRtos, task::block_on}, sys::time};
    ...
    fn assert_timer_driver_error<T>(res: Result<T, TimerDriverError>, expected_err: TimerDriverError){
    ...
    fn get_base_timer_driver<'a>()->(TimerDriver<'a>, Notification){
        ...
        (TimerDriver::new(timer_group, notif.notifier()).unwrap(), notif)
    }

    fn update_after_interrupt<'a>(timer_driver: &mut TimerDriver<'a>, notif: &Notification)-> Result<(), Esp32FrameworkError>{
        notif.blocking_wait();
    ...
    fn counter_to_micro<'a>(timer_driver: &mut TimerDriver<'a>, counter: u64) -> u64 {
        counter * MICRO_IN_SEC / timer_driver.inner.deref().driver.tick_hz()
    }
    ...
    fn timer_driver_02_creating_with_taken_timer_group(){
        ...
        assert_timer_driver_error(res, TimerDriverError::InvalidPeripheral(PeripheralError::AlreadyTaken));
    }
    ...
    fn timer_driver_03_creating_with_non_timer_group_peripheral(){
        ...
        assert_timer_driver_error(res, TimerDriverError::InvalidPeripheral(PeripheralError::NotATimerGroup));
    }
    ...
    fn timer_driver_11_interrupt_multiple_times_first_tick_happens_after_the_time_has_passed(){
        ...
        timer_driver.interrupt_after_n_times(NEVER, None, true, move || {*executed_callback_ref.deref_mut() = true});
        timer_driver.enable().unwrap();
    ...
    fn timer_driver_12_interrupt_multiple_times(){
        ...
        timer_driver.interrupt_after_n_times(100, Some(3), true, move || {*amount_of_callbacks_ref.deref_mut()+=1});
        timer_driver.enable().unwrap();
    ...
    fn timer_driver_13_interrupt_multiple_times_no_reenable(){
        ...
        timer_driver.interrupt_after_n_times(100, Some(3), false, move || {*amount_of_callbacks_ref.deref_mut()+=1});
        for _ in 0..2{
            timer_driver.enable().unwrap();
            update_after_interrupt(&mut timer_driver, &notif).unwrap();
        }
        ...
        FreeRtos::delay_ms(1);
        notif.notifier().notify();
        update_after_interrupt(&mut timer_driver, &notif);
