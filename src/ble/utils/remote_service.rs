...
impl _RemoteCharacteristic{
    ...
    pub async fn read_async(&mut self) -> Result<Vec<u8> ,BleError>{
        ...
        self.characteristic.read_value().await.map_err(BleError::from_characteristic_context)
    }
    ...
    pub async fn get_descriptor_async(&mut self, id: &BleId) -> Result<RemoteDescriptor, BleError>{
        let remote_descriptor = self.characteristic.get_descriptor(id.to_uuid()).await.map_err(BleError::from_descriptors_context)?;
...
impl RemoteDescriptor{
    ...
    /// Non blocking async version of [Self::read]
    pub async fn read_async(&mut self)-> Result<Vec<u8>, BleError>{
        self.descriptor.read_value().await.map_err(BleError::from_descriptors_context)
    }
    ...
    /// Non blocking async version of [Self::write]
    pub async fn write_async(&mut self, data: &[u8]) -> Result<(), BleError> {
        self.descriptor.write_value(data, true).await.map_err(BleError::from_descriptors_context)
    }
